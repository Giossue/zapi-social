...
**Fases 1 a 7 completadas.** La superficie canónica existe en `../template-shadcn-superdashboard/src/app/(main)/dashboard/rss-schedules/` y fue copiada a Portal V2 en `apps/web/features/rss-schedules/`. El listado, búsqueda/filtro/estados y wizard de cuatro pasos conservan la composición canónica, pero ahora consumen REST: la tabla filtra y pagina en servidor, el wizard resuelve canales conectados y zona horaria de perfil, y crear, pausar, reactivar, eliminar y ejecutar ya son acciones durables. La tabla conserva el mismo avatar, menú contextual compacto y footer de paginación de Channels/Captions, también cuando no hay resultados. El Worker revisa las programaciones cada minuto y transforma cada artículo nuevo en un borrador por canal, sin publicar automáticamente. La migración `0017` está aplicada tanto localmente como en la base remota; queda como validación operativa posterior crear una programación real y observar su primer borrador.
...
No existe una pantalla RSS equivalente en `template-shadcn-superdashboard`; por tanto se creará primero, desde cero, en `../template-shadcn-superdashboard/src/app/(main)/dashboard/rss-schedules/`, usando exclusivamente sus componentes ya instalados:
...
- Fase 1: `biome check` y `next build` ejecutados en `../template-shadcn-superdashboard`; la ruta canónica `/dashboard/rss-schedules` compiló correctamente. Tras la iteración de tabla basada en Channels y la posterior validación del wizard, `biome check` focal de sus componentes y nuevos `next build` también pasaron. El `biome check` global continúa teniendo errores preexistentes y ajenos en Audit y File Manager.
