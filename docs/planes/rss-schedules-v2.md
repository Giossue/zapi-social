...
**Fases 1 a 6 completadas.** La superficie canónica existe en `../diseño ideal/src/app/(main)/dashboard/rss-schedules/` y fue copiada a Portal V2 en `apps/web/features/rss-schedules/`. El listado, búsqueda/filtro/estados y wizard de cuatro pasos conservan la composición canónica, pero ahora consumen REST: la tabla filtra y pagina en servidor, el wizard resuelve canales conectados y zona horaria de perfil, y crear, pausar, reactivar, eliminar y ejecutar ya son acciones durables. La iteración visual vigente usa la misma jerarquía de tabla operativa de Channels: una card que contiene cabecera, búsqueda, acción principal, filtros, tabla y paginación; las celdas muestran un dato principal y su contexto secundario, y las mutaciones viven en un menú contextual único. El Worker revisa las programaciones cada minuto y transforma cada artículo nuevo en un borrador por canal, sin publicar automáticamente. Quedan las pruebas de deduplicación/ownership y el cierre documental final.
...
7. Añadir auditoría, pruebas focales de ownership/deduplicación y documentación de cierre.
...
- Fase 2: `bun run typecheck` y `bun run build` ejecutados en `apps/web`; `/portal/rss-schedules` aparece en el build. Tras copiar la nueva composición y la validación del wizard, `prettier --check` focal, `bun run typecheck` y nuevos `bun run build` también pasaron. `bun run lint` finalizó sin errores; mantiene warnings preexistentes ajenos a RSS Schedules.
- Fases 3 y 4: `bun run typecheck` global pasó; el parseo de contratos confirmó defaults, orden de slots/días y rechazo de FTP/horarios repetidos. Drizzle generó `0017_rss_schedules`, que se probó dentro de una transacción revertida y luego se aplicó solo a `zapi_v2_local` con el rol `zapi_social`. Se verificaron las cuatro tablas vacías, FKs e índices, incluida la unicidad de deduplicación. No se ejecutó ninguna migración remota.
...
- Fase 6: `apps/worker` pasa `bun run typecheck` y `bun run build`. La prueba focal `rss-schedule-time.spec.ts` pasa 3 casos: zona horaria persistida, siguiente día habilitado y límite de fecha final. El job repetible, la recuperación de runs en cola, la creación de borradores y sus auditorías se implementaron sin tocar la base remota. Faltan pruebas focales que ejecuten la transacción real de deduplicación y la separación de workspace antes de cerrar la fase 7.
