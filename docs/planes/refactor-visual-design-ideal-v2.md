...
- Política de datos del 24 de agosto de 2026: actividad, KPI y distribución de Publishing cuentan exclusivamente posts `published` usando `publishedAt`; la distribución agrupa por canal real (Instagram/Facebook/WhatsApp), no por integración técnica (`meta`). AI muestra generaciones `succeeded`, Files solo assets `ready`, canales activos excluyen desconectados y Próximas limita su cola a `scheduled` futuros más borradores. Los fallos siguen visibles en sus módulos operativos, pero no se presentan como resultados válidos en Portal ni Admin. Validación: la consulta remota devuelve un único post `published` agrupado como `instagram`, los dos assets como `ready`; 95 pruebas API, typecheck/lint de API/Web y ambas auditorías i18n aprobados.

## Dashboard Portal — prioridad operativa de Publishing — 29 de agosto de 2026

- El resumen deja de usar `Archivos listos` y `Créditos AI usados` como KPIs principales. Ahora muestra publicaciones completadas en 28 días, publicaciones previstas para las próximas 24 horas, borradores pendientes y canales activos listos para publicar.
- El contrato expone el estado actual de cada publicación (`draft`, `scheduled`, `processing`, `published`, `failed`) y el origen de las publicaciones completadas de las últimas cuatro semanas. Ambas consultas se filtran por `workspaceId` de la sesión; el bloque AI permanece secundario y solo aparece al haber uso real.
- La composición canónica en `template-shadcn-superdashboard/src/app/(main)/dashboard/default/` incorpora un donut de estado junto a la tendencia, un gráfico de origen de contenido junto a Próximas publicaciones y traslada Uso de AI a una fila inferior. V2 copia la misma jerarquía con datos, textos y permisos reales.
- Validación: build de Web, typecheck de Contracts y API, auditorías de Portal/UI e i18n y `git diff --check` correctos. La fuente pasa Biome focal; su build global conserva dos errores preexistentes fuera de esta superficie en `default-v1` y `ecommerce`.
