...
- El selector inicial de capabilities sustituye su grid/card V2 por la jerarquía de `diseño ideal/src/app/(main)/dashboard/file-manager/_components/folders-section.tsx`: `Card`, `CardHeader`, icono, título, descripción, acción compacta y pie de contenido. Conserva availability, bloqueo y el callback de conexión Zapi.
- Divergencia indispensable: el selector usa un máximo de dos columnas dentro del diálogo en lugar de las cuatro del grid de archivos fuente; evita comprimir títulos, descripciones y acciones cuando el diálogo no ocupa el ancho de una página.
