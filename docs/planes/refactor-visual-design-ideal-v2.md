...
- Divergencia frente a la fuente: V2 usa `DataTableSearch` donde la fuente compone `InputGroup` a mano, porque el pattern compartido produce el mismo control y evita duplicarlo; y el tipado estricto de V2 obliga a resolver la vista por constante en vez de `views[0].key`.
