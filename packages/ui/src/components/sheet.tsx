...
function SheetOverlay({
  ...
  return (
    <SheetPrimitive.Overlay
      ...
      className={cn(
        "fixed inset-0 z-50 bg-black/40 data-open:animate-[overlay-fade-in_200ms_ease-out] data-closed:animate-[overlay-fade-out_200ms_ease-in]",
        className
      )}
      {...props}
    />
...
function SheetActions({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "mt-auto flex shrink-0 flex-row justify-end gap-2 border-t bg-popover p-4 pb-[max(1rem,env(safe-area-inset-bottom))]",
        className
      )}
      {...props}
    />
