import { Catch, type ArgumentsHost, HttpException, type ExceptionFilter, HttpStatus } from '@nestjs/common'
...
export class AppExceptionFilter implements ExceptionFilter {
  ...
  private resolveException(exception: unknown): { status: number; code: AppErrorCode } {
    ...
    if (exception instanceof HttpException) {
      ...
      return {
        status,
        code: status === HttpStatus.BAD_REQUEST ? 'VALIDATION_FAILED' : 'REQUEST_FAILED',
      }
    }
