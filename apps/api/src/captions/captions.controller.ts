...
export class CaptionsController {
  ...
  async list(@Req() request: FastifyRequest, @Query() query: unknown) {
    return this.captions.list(await this.access.requirePortalSession(request), query)
  }

  @Post()
  async create(@Req() request: FastifyRequest, @Body() body: unknown) {
    return this.captions.create(await this.access.requirePortalSession(request), body)
  }

  @Patch(':id')
  async update(@Req() request: FastifyRequest, @Param('id') id: string, @Body() body: unknown) {
    return this.captions.update(await this.access.requirePortalSession(request), id, body)
  }

  @Delete(':id')
  @HttpCode(HttpStatus.NO_CONTENT)
  async remove(@Req() request: FastifyRequest, @Param('id') id: string) {
    await this.captions.remove(await this.access.requirePortalSession(request), id)
  }
}
