...
export class CaptionsService {
  ...
  async list(
    ...
    query: unknown,
  ): Promise<PortalCaptionsResponse> {
    ...
    if (parsed.data.sourceType)
      filters.push(eq(captions.sourceType, parsed.data.sourceType));
    if (parsed.data.status)
      filters.push(eq(captions.status, parsed.data.status));
    if (parsed.data.q) {
      const value = `%${parsed.data.q}%`;
      filters.push(
        or(
          ilike(captions.name, value),
          ilike(captions.content, value),
          ilike(captions.notes, value),
        )!,
      );
    }
    ...
    const [rows, metricRows] = await Promise.all([
      this.database.db
        ...
        .from(captions)
        .where(and(...filters))
        .orderBy(desc(captions.updatedAt), desc(captions.id)),
      this.database.db
        .select()
        .from(captions)
        .where(eq(captions.workspaceId, session.workspace.id)),
    ]);
    ...
    return {
      ...
      metrics: {
        ...
        manual: metricRows.filter((caption) => caption.sourceType === 'manual')
          .length,
        active: metricRows.filter((caption) => caption.status === 'active')
          .length,
      },
    };
  }
      ...
      const [updated] = await tx
        ...
        .where(
          and(
            ...
            eq(captions.workspaceId, session.workspace.id),
          ),
        )
        .returning();
  ...
  private async findForWorkspace(workspaceId: string, id: string) {
    const [caption] = await this.database.db
      .select()
      .from(captions)
      .where(and(eq(captions.id, id), eq(captions.workspaceId, workspaceId)))
      .limit(1);
    if (!caption) throw this.notFound();
    return caption;
  }
  ...
  private async uniqueSlug(
    ...
    while (true) {
      const [existing] = await this.database.db
        ...
        .from(captions)
        .where(
          and(eq(captions.workspaceId, workspaceId), eq(captions.slug, slug)),
        )
        .limit(1);
