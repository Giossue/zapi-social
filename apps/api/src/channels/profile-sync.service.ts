...
export class ProfileSyncService {
  ...
  async requestManualSync(session: PortalAuthSession, id: string) {
    ...
    const account = await this.accountForWorkspace(session.workspace.id, accountId)
    const providerKey = this.supportedProvider(account)
    const acceptedAt = new Date()
    const nextAllowedAt = new Date(acceptedAt.getTime() + MANUAL_REFRESH_COOLDOWN_MS)
    const previousRequestedAt = this.requestedAt(account.metadata)

    if (previousRequestedAt && previousRequestedAt.getTime() > acceptedAt.getTime() - MANUAL_REFRESH_COOLDOWN_MS) {
      throw new AppException('CHANNEL_PROFILE_SYNC_COOLDOWN', HttpStatus.TOO_MANY_REQUESTS)
    }
    ...
    if (!reserved) {
      await this.throwReservationFailure(session.workspace.id, accountId, acceptedAt)
    }
    ...
    try {
      ...
      throw new AppException('INFRASTRUCTURE_UNAVAILABLE', HttpStatus.SERVICE_UNAVAILABLE)
    }
  ...
  private async accountForWorkspace(workspaceId: string, accountId: string): Promise<SocialAccount> {
  ...
  private supportedProvider(account: SocialAccount): SupportedProviderKey {
    if (account.status !== 'active') {
      throw new AppException('CHANNEL_PROFILE_SYNC_UNAVAILABLE', HttpStatus.CONFLICT)
    }
    if (account.providerKey === 'meta') return 'meta'
    if (account.providerKey === 'whatsapp-status') return 'whatsapp-status'
    throw new AppException('CHANNEL_PROFILE_SYNC_UNSUPPORTED', HttpStatus.CONFLICT)
  }
  ...
  private cooldownAvailable(now: Date) {
    const threshold = new Date(now.getTime() - MANUAL_REFRESH_COOLDOWN_MS).toISOString()
    return sql`(
  ...
  private async throwReservationFailure(workspaceId: string, accountId: string, now: Date): Promise<never> {
    ...
    if (requestedAt && requestedAt.getTime() > now.getTime() - MANUAL_REFRESH_COOLDOWN_MS) {
      throw new AppException('CHANNEL_PROFILE_SYNC_COOLDOWN', HttpStatus.TOO_MANY_REQUESTS)
    }
    throw new AppException('CHANNEL_PROFILE_SYNC_UNAVAILABLE', HttpStatus.CONFLICT)
  }

  private async revertReservation(accountId: string, acceptedAt: Date, previousRequestedAt: Date | null) {
