...
import { channelConnectionSessions, socialAccounts } from '@workspace/database';
import {
...
export class WhatsAppStatusConnectionsService {
  ...
  async start(session: PortalAuthSession, input: unknown) {
    ...
    const values = this.parse(
      startWhatsAppStatusConnectionSchema.safeParse(input),
    );
    const configuration =
      await this.integrations.readWhatsAppStatusConfiguration();
    const reconnect = values.reconnectAccountId
      ? await this.reconnectAccount(session, values.reconnectAccountId)
      : null;
    const reconnectDeviceId = reconnect
      ? this.deviceId(reconnect.metadata)
      : null;

    let deviceId = reconnectDeviceId ?? randomUUID();
    let mustCreateDevice = !reconnectDeviceId;
    if (reconnectDeviceId) {
      const purgeResult = await this.purgeDevice(
        configuration,
        reconnectDeviceId,
      );
    ...
    try {
      ...
      if (error instanceof GoWaConnectorError)
  ...
  async refreshQr(session: PortalAuthSession, id: string) {
    ...
    return this.withDeviceLock(deviceId, async () => {
      ...
      if (
        !['qr_ready', 'waiting_for_scan', 'expired'].includes(current.status)
      ) {
        throw new BadRequestException();
      }
      if (current.externalConnectionId !== deviceId)
  ...
  async status(session: PortalAuthSession, id: string) {
    ...
    if (connection.status === 'connected') {
      return {
        ...
        account: await this.connectionAccount(connection),
        publicError: null,
      };
    }
    ...
    return this.withDeviceLock(deviceId, async () => {
      ...
      if (current.status === 'connected') {
        return {
          connection: this.serializeConnection(current),
          account: await this.connectionAccount(current),
          publicError: null,
        };
      }
      const configuration =
        await this.integrations.readWhatsAppStatusConfiguration();
      try {
        const providerStatus = await this.requestJson(
          configuration,
        ...
        const account = await this.persistConnectedDevice(
          current,
          configuration,
          providerStatus,
        );
        ...
        if (error instanceof ConflictException) {
          await this.purgeDevice(configuration, deviceId).catch(
            () => undefined,
          );
        }
        const updated = await this.updateConnection(current.id, {
          ...
          publicError: {
            code:
              error instanceof ConflictException
                ? 'WHATSAPP_ACCOUNT_MISMATCH'
                : 'WHATSAPP_CONNECTOR_UNAVAILABLE',
  ...
  async qr(session: PortalAuthSession, id: string) {
    ...
    const connection = await this.connectionForSession(session, id);
    if (
      !['qr_ready', 'waiting_for_scan'].includes(connection.status) ||
      connection.expiresAt <= new Date()
    ) {
      throw new NotFoundException();
    }
    const context = this.decryptContext(connection);
    if (context.qrLink.startsWith('data:image/'))
      return this.dataQr(context.qrLink);
    ...
    try {
      ...
      if (!contentType.startsWith('image/'))
        throw new Error('Unexpected QR content type');
  ...
  async cancel(
    ...
    id: string,
  ): Promise<PortalChannelConnection> {
    this.requireManager(session);
    const connection = await this.connectionForSession(session, id);
    ...
    return this.serializeConnection(
      await this.updateConnection(connection.id, { status: 'cancelled' }),
    );
  }
  ...
  async purgeAccountDevice(account: typeof socialAccounts.$inferSelect) {
    if (
      ...
      account.capabilityKey !== capabilityKey
    )
      return;
    const deviceId = this.deviceId(account.metadata);
    if (!deviceId) return;
    await this.purgeDevice(
      await this.integrations.readWhatsAppStatusConfiguration(),
      deviceId,
    );
  }

  private async persistConnectedDevice(
    ...
    const [previous] = connection.reconnectAccountId
      ? await this.database.db
          .select()
          .from(socialAccounts)
          .where(eq(socialAccounts.id, connection.reconnectAccountId))
          .limit(1)
      : [];
    const previousPhone = previous
      ? this.phoneIdentity(this.firstString(previous.metadata, ['phoneNumber']))
      : null;
  ...
  private async profile(
    ...
    const phoneNumber =
      this.firstString(info, [
        'results.phone_number',
        'results.phone',
        'results.jid',
      ]) ??
      this.firstString(device, [
        'results.phone_number',
        'results.phone',
        'results.jid',
      ]) ??
      this.firstString(status, [
        'results.phone_number',
        'results.phone',
        'results.jid',
      ]);
    const avatar = phoneNumber
      ? await this.requestJson(
          configuration,
          `/user/avatar?phone=${encodeURIComponent(phoneNumber)}&is_preview=false&is_community=false`,
          deviceId,
        ).catch(() => ({}))
      : {};
    return {
  ...
  private async createDevice(
    ...
    return (
      this.firstString(result, ['results.id', 'results.device_id']) ?? deviceId
    );
  }

  private async startQr(
    configuration: GoWaConfiguration,
    deviceId: string,
  ): Promise<QrStart> {
    const attempts = [
    ...
    for (const request of attempts) {
      try {
        ...
        const qrLink = this.firstString(result, [
          'results.qr_link',
          'results.qr_url',
          'results.qr',
          'results.qrcode',
        ...
        const durationSeconds = this.firstNumber(result, [
          'results.qr_duration',
        ]);
        if (
          qrLink &&
          durationSeconds !== null &&
          durationSeconds >= minQrDurationSeconds &&
          durationSeconds <= maxQrDurationSeconds
        ) {
  ...
  private async purgeDevice(
    configuration: GoWaConfiguration,
    deviceId: string | null,
  ): Promise<PurgeDeviceResult | null> {
    if (!deviceId) return null;
    try {
      await this.requestJson(
        configuration,
        `/devices/${encodeURIComponent(deviceId)}/logout`,
        undefined,
        { method: 'POST' },
      );
      return 'preserved';
  ...
  private isLoggedIn(value: GoWaResponse) {
    const state = this.firstString(value, [
      'results.status',
      'results.state',
  ...
  private isGoWaQrPath(pathname: string) {
    return /(?:^|\/)statics\/qrcode\/scan-qr-[a-zA-Z0-9-]+\.png$/.test(
      pathname,
    );
  }
  ...
  private dataQr(value: string) {
    ...
    const contentType = metadata.match(
  ...
  private async connectionAccount(connection: ConnectionRow) {
    ...
    const [account] = await this.database.db
      .select()
      .from(socialAccounts)
      .where(eq(socialAccounts.id, connection.socialAccountId))
      .limit(1);
  ...
  private async updateConnection(
    id: string,
    values: Partial<typeof channelConnectionSessions.$inferInsert>,
  ) {
    return (
      await this.database.db
        .update(channelConnectionSessions)
        .set({ ...values, updatedAt: new Date() })
        .where(eq(channelConnectionSessions.id, id))
        .returning()
    )[0];
  }
  ...
  private encryptContext(deviceId: string, context: ConnectionContext) {
    return this.encryption().encrypt(
      ...
      `whatsapp-status:${deviceId}`,
    );
  }

  private decryptContext(connection: ConnectionRow): ConnectionContext {
    if (!connection.contextCiphertext || !connection.externalConnectionId)
      throw new BadRequestException();
    try {
      const value: unknown = JSON.parse(
        this.encryption().decrypt(
          connection.contextCiphertext,
          `whatsapp-status:${connection.externalConnectionId}`,
        ),
      );
      if (
        !value ||
        typeof value !== 'object' ||
        typeof (value as ConnectionContext).qrLink !== 'string'
      )
  ...
  private serializeConnection(
    ...
    return {
      id: connection.id,
      capabilityKey,
      state: connection.status as PortalChannelConnection['state'],
  ...
  private serializeAccount(
    ...
    return {
      ...
      externalName:
        typeof account.metadata.externalDisplayName === 'string'
  ...
  private async withDeviceLock<T>(
    ...
    try {
      if ((await redis.set(key, token, 'PX', 10_000, 'NX')) !== 'OK')
        throw new ConflictException();
