...
export class ChannelConnectionsService {
  ...
  async start(session: PortalAuthSession, input: unknown) {
    ...
    if (values.reconnectAccountId) {
      await this.assertReconnectAccount(
        session,
  ...
  async candidates(session: PortalAuthSession, connectionId: string) {
    ...
    const rows = await this.database.db
      ...
      .where(
        eq(
          channelConnectionCandidates.channelConnectionSessionId,
          connection.id,
        ),
      );
    return {
  ...
  async select(
    ...
    this.requireManager(session);
    const candidateId = this.parse(
      selectPortalChannelCandidateSchema.safeParse(input),
    ).candidateId;
    const connection = await this.connectionForSession(session, connectionId);
    if (connection.status !== 'picker_ready') throw new BadRequestException();
    ...
    if (!selected || selected.externalId !== candidate.externalId)
      throw new BadRequestException();

    const account = await this.persistSelection(
      connection,
      selected,
      context.scopes,
    );
  ...
  async callback(query: unknown): Promise<{
    ...
    try {
      if (!this.isMetaCapability(connection.capabilityKey))
        throw new BadRequestException();
      const configuration =
        await this.integrations.readOAuthConfiguration('facebook');
      ...
      const inserted = candidates.length
        ? await this.database.db
            ...
            .returning({
              id: channelConnectionCandidates.id,
              externalId: channelConnectionCandidates.externalId,
            })
        : [];
  ...
  private async persistSelection(
    ...
    await this.database.db
      ...
      .values({
        socialAccountId: account.id,
        accessTokenCiphertext: this.encryption().encrypt(
  ...
  private async exchangeCode(
    ...
    if (!response.ok || !this.hasAccessToken(body))
  ...
  private async fetchCandidates(userAccessToken: string) {
    ...
    const body: unknown = await response.json().catch(() => null);
    if (!response.ok || !this.hasPages(body))
      throw new ServiceUnavailableException();
      ...
      pageCandidate,
      {
        ...
        context: {
          ...
          handle: instagram.username
            ? instagram.username.replace(/^@+/, '') || null
            : null,
          profileUrl: instagram.username
  ...
  private async accountForWorkspace(session: PortalAuthSession, id: string) {
    const [account] = await this.database.db
      ...
      .limit(1);
    if (!account || !this.isMetaCapability(account.capabilityKey))
  ...
  private async assertReconnectAccount(
    ...
    if (account.capabilityKey !== capabilityKey)
      throw new BadRequestException();
  }
  ...
  private serializeConnection(
    ...
    return {
      id: connection.id,
      capabilityKey:
        connection.capabilityKey as PortalChannelConnection['capabilityKey'],
      state,
  ...
  private parseCallback(value: unknown) {
    ...
    if (query.code !== undefined && typeof query.code !== 'string')
      throw new BadRequestException();
    if (query.error !== undefined && typeof query.error !== 'string')
      throw new BadRequestException();
