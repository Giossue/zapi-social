  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (caught instanceof ApiError) {
        console.error('Auth request failed', { code: caught.code, requestId: caught.requestId })
        toast.error(authErrorMessages[caught.code] ?? 'No pudimos completar la solicitud. Inténtalo de nuevo.')
      } else {
    ...
    <div ref={rootRef}>
      <div ref={cardRef}>
        <Card variant="surface" className="gap-5 overflow-hidden py-5">
          <CardHeader className="gap-3">
            <div className="relative grid grid-cols-2 rounded-lg border border-border bg-muted/40 p-1" aria-label="Modo de autenticación">
              <span
                ref={indicatorRef}
                aria-hidden="true"
        ...
        <div className="space-y-1">
          <CardTitle>{mode === 'login' ? 'Bienvenido de nuevo' : 'Crea tu cuenta'}</CardTitle>
          <CardDescription>
            {mode === 'login'
      ...
      <CardContent>
        <form noValidate onSubmit={submit}>
          <div
            className={`grid overflow-hidden transition-[grid-template-rows,margin] duration-500 ease-out motion-reduce:transition-none ${mode === 'register' ? 'grid-rows-[1fr] mb-4' : 'grid-rows-[0fr] mb-0'}`}
          >
            <div className="min-h-0 overflow-hidden">
              <label className="grid gap-1.5 text-sm font-medium text-foreground" htmlFor="display-name">
                <span>Nombre <span className="text-destructive" aria-hidden="true">*</span></span>
                <span className="relative">
                  ...
                  <Input id="display-name" name="displayName" className="pl-9" autoComplete="name" required={mode === 'register'} minLength={2} maxLength={160} tabIndex={mode === 'register' ? 0 : -1} />
                </span>
              </label>
            </div>
          </div>
          <label className="mb-4 grid gap-1.5 text-sm font-medium text-foreground" htmlFor="email">
            <span>Correo electrónico <span className="text-destructive" aria-hidden="true">*</span></span>
            <span className="relative">
              <Mail className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" />
              <Input id="email" name="email" type="email" className="pl-9" autoComplete="email" required maxLength={320} />
            </span>
          </label>
          <label className="mb-4 grid gap-1.5 text-sm font-medium text-foreground" htmlFor="password">
            <span>Contraseña <span className="text-destructive" aria-hidden="true">*</span></span>
            <span className="relative">
          ...
          >
            <div className="min-h-0 overflow-hidden">
              <div className="grid gap-4">
                <label className="grid gap-1.5 text-sm font-medium text-foreground" htmlFor="password-confirmation">
                  <span>Confirmar contraseña <span className="text-destructive" aria-hidden="true">*</span></span>
                  <span className="relative">
                    <LockKeyhole className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" />
                    <Input
          ...
          <Button type="submit" size="lg" className="w-full" disabled={isSubmitting}>
            {isSubmitting ? 'Comprobando…' : mode === 'login' ? 'Entrar al portal' : 'Crear cuenta'}
          </Button>
        </form>
      </CardContent>
    </Card>
