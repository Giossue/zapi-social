...
import type { Caption, CaptionDraft, CaptionFilters, CaptionMetrics } from "@/features/captions/types/captions"
...
export function filterCaptionsMock(captions: readonly Caption[], filters: CaptionFilters): Caption[] {
  ...
  return captions.filter((caption) => {
    const matchesQuery =
      ...
      [caption.name, caption.content, caption.notes ?? "", ...caption.tags]
        ...
        .includes(query)
    const matchesSource = filters.sourceType === "all" || caption.sourceType === filters.sourceType
    const matchesStatus = filters.status === "all" || caption.status === filters.status

    return matchesQuery && matchesSource && matchesStatus
  })
}

export function getCaptionMetricsMock(captions: readonly Caption[]): CaptionMetrics {
...
export async function createCaptionMock(captions: readonly Caption[], draft: CaptionDraft): Promise<Caption> {
  return {
    ...draft,
...
export async function updateCaptionMock(caption: Caption, draft: CaptionDraft): Promise<Caption> {
