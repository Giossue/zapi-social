...
const blockTouchResizerClass =
  "absolute size-2 border border-(--fc-event-color) bg-card rounded-full"
const rowTouchResizerClass = `${blockTouchResizerClass} top-1/2 -mt-1`
const columnTouchResizerClass = `${blockTouchResizerClass} left-1/2 -ml-1`

const tallDayCellBottomClass = "min-h-3"
const getShortDayCellBottomClass = (info: DayCellInfo) =>
  cn(!info.isNarrow && "min-h-0.5")

const dayRowCommonClasses: CalendarOptions = {
  listItemEventClass: (info) =>
    cn(
      "mb-px rounded-sm p-px",
  ...
  listItemEventTimeClass: (info) =>
    cn(
      ...
      "order-1 shrink-1 overflow-hidden whitespace-nowrap"
    ),
  listItemEventTitleClass: (info) =>
    cn(
      info.isNarrow ? "px-px" : "px-0.5",
      "shrink-100 overflow-hidden font-medium whitespace-nowrap",
  ...
  rowMoreLinkClass: (info) =>
    cn(
      ...
      info.isNarrow
        ...
        : "mx-1 self-start border-transparent bg-foreground/5 hover:bg-foreground/10"
    ),
  rowMoreLinkInnerClass: (info) =>
    cn(info.isNarrow ? `p-px ${xxsTextClass}` : "p-0.5 text-xs"),
}
  ...
  return (
    <FullCalendar
      ...
      blockEventClass={(info) =>
        cn(
          "group relative border-transparent bg-(--fc-event-color) hover:bg-[color-mix(in_oklab,var(--fc-event-color)_92%,var(--fc-event-contrast-color))] print:border-(--fc-event-color) print:bg-white",
          info.isInteractive &&
      ...
      rowEventTitleClass={(info) =>
        cn(info.isNarrow ? "px-0.5" : "px-1", "font-medium")
      }
      ...
      columnEventClass={(info) =>
        cn(
          ...
          info.isStart && [
            ...
            info.isNarrow ? "mt-px" : "mt-0.5",
          ],
          info.isEnd && [
            "rounded-b-lg border-b",
            info.isNarrow ? "mb-px" : "mb-0.5",
          ]
        )
      }
      ...
      columnEventTimeClass={(info) =>
        cn(!info.isShort && (info.isNarrow ? "pt-0.5" : "pt-1"))
      }
        ...
        cn(
          ...
          info.inPopover
            ...
            : !info.dayNumberText
              ...
              : !info.isToday
                ? [
                    "mx-2 my-2.5 h-6 rounded-sm px-1.5 text-muted-foreground",
                    info.hasNavLink && "hover:bg-foreground/5",
                  ]
              ...
              <span
                ...
                className={cn(
                  ...
                  textPart.type === "day" && info.isToday
                    ? [
                        "flex size-7 flex-row items-center justify-center rounded-full bg-chart-1/50 font-semibold first:-ms-1 last:-me-1",
                        info.hasNavLink &&
                          "outline-ring/50 group-hover:bg-chart-1/60 group-focus-visible:outline-3",
                      ]
            ...
            {info.textParts.map((textPart, i) => (
              <span
                ...
                className={cn(
                  ...
                  textPart.type === "day" && info.isToday
                    ? [
                        "flex flex-row items-center justify-center rounded-full bg-chart-1/50 font-semibold",
                        info.hasNavLink &&
                          "outline-ring/50 group-hover:bg-chart-1/60 group-focus-visible:outline-3",
                        info.isNarrow
                          ? "size-5"
      ...
      listDayHeaderInnerClass={(info) =>
        cn(
          "m-1.5 rounded-sm px-1.5 py-0.5 text-sm",
          !info.level && "font-semibold",
          !info.level && info.isToday
            ? [
                "bg-chart-1/50",
                info.hasNavLink &&
                  "outline-ring/50 hover:bg-chart-1/60 focus-visible:outline-3",
              ]
            : info.hasNavLink && "hover:bg-foreground/5"
        )
      }
      ...
      singleMonthHeaderClass={(info) =>
        cn(
          info.multiMonthColumns > 1 ? "pb-2" : "border-b bg-card py-1",
          "items-center"
        )
      }
      singleMonthHeaderInnerClass={(info) =>
        cn(
          "rounded-sm px-1.5 py-0.5 text-base font-semibold",
          info.hasNavLink && "hover:bg-foreground/5"
        )
      }
      ...
      inlineWeekNumberClass={(info) =>
        cn(
          "absolute start-0 rounded-e-sm whitespace-nowrap text-muted-foreground",
          info.isNarrow
      ...
      views={{
        ...
        multiMonth: {
          ...
          dayHeaderDividerClass: (info) =>
            cn(info.multiMonthColumns === 1 && "border-b"),
          dayCellBottomClass: getShortDayCellBottomClass,
          tableBodyClass: (info) =>
            cn(
              info.multiMonthColumns > 1 && "overflow-hidden rounded-sm border"
            ),
        ...
        timeGrid: {
          ...
          dayHeaderDividerClass: (info) =>
            cn(
              ...
              !info.options.allDaySlot &&
                "border-foreground/20 not-print:shadow-sm"
            ),
          ...
          weekNumberHeaderInnerClass: (info) =>
            cn(
              "mx-0.5 flex h-6 flex-row items-center rounded-sm px-1.5 text-muted-foreground",
              info.isNarrow ? "text-xs" : "text-sm",
              info.hasNavLink && "hover:bg-foreground/5"
            ),
        ...
        list: {
          listItemEventClass: (info) =>
            cn(
              "group rounded-sm py-1 hover:bg-muted/50",
              info.isInteractive &&
                "-outline-offset-3 focus-visible:bg-muted/50"
            ),
          ...
          listItemEventTitleClass: (info) =>
            cn(
              "min-w-0 grow overflow-hidden px-4 py-2 text-sm whitespace-nowrap",
