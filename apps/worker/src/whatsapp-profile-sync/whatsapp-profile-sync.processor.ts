...
@Injectable()
@Processor(WHATSAPP_PROFILE_SYNC_QUEUE, {
  concurrency: WHATSAPP_PROFILE_SYNC_CONCURRENCY,
})
export class WhatsAppProfileSyncProcessor extends WorkerHost {
  ...
  async process(job: Job<WhatsAppProfileSyncJobData>): Promise<void> {
    ...
    try {
      ...
      const errorCode =
        error instanceof WhatsAppProfileSyncError
          ? error.code
          : 'WHATSAPP_PROFILE_SYNC_FAILED';
      this.logger.error(
        {
  ...
  private async readConfiguration(): Promise<GoWaConfiguration> {
    const integrations = await this.database.client<
    ...
    if (!ciphertext)
      throw new WhatsAppProfileSyncError('INTEGRATION_NOT_READY');
  ...
  private async fetchProfile(
    ...
    const info = await this.requestJson(
      configuration,
      '/user/info',
      deviceId,
    ).catch(() => ({}));
    ...
    const infoPhone = this.firstString(info, [
      'results.phone_number',
      'results.phone',
      'results.jid',
    ]);
    const knownPhone =
      this.firstString(account.metadata, ['phoneNumber']) ?? account.handle;

    const device =
      !infoDisplayName || !infoPhone
  ...
  private async persistProfile(
    ...
    const nextMetadata = {
      ...
      profileSyncDueAt: new Date(
        now.getTime() + WHATSAPP_PROFILE_SYNC_DUE_INTERVAL_MS,
      ).toISOString(),
    };
  ...
  private async requestJson(
    ...
    try {
      response = await fetch(
        new URL(path.replace(/^\/+/, ''), `${configuration.baseUrl}/`),
        {
          headers: {
    ...
    if (response.status === 401 || response.status === 403)
      throw new WhatsAppProfileSyncError('GOWA_UNAUTHORIZED');
    if (response.status === 429)
      throw new WhatsAppProfileSyncError('GOWA_RATE_LIMITED');
    if (!response.ok)
      throw new WhatsAppProfileSyncError('GOWA_REQUEST_REJECTED');
    if (!this.isSuccess(body))
      throw new WhatsAppProfileSyncError('GOWA_RESPONSE_INVALID');
    return body;
  }
  ...
  private parseConfiguration(value: unknown): GoWaConfiguration | null {
    ...
    const basicAuthUsername =
      typeof value.basicAuthUsername === 'string'
        ? value.basicAuthUsername.trim()
        : '';
    const basicAuthPassword =
      typeof value.basicAuthPassword === 'string'
        ? value.basicAuthPassword.trim()
        : '';
  ...
  private deviceId(account: WhatsAppAccount): string | null {
    ...
    if (
      typeof metadataDeviceId === 'string' &&
      metadataDeviceId.length > 0 &&
      metadataDeviceId.length <= 255
    ) {
      return metadataDeviceId;
    }
    return account.externalId && account.externalId.length <= 255
      ? account.externalId
      : null;
  }
