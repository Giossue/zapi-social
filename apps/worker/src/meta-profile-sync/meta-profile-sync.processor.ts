...
@Injectable()
@Processor(META_PROFILE_SYNC_QUEUE, { concurrency: META_PROFILE_SYNC_CONCURRENCY })
export class MetaProfileSyncProcessor extends WorkerHost {
  ...
  async process(job: Job<MetaProfileSyncJobData>): Promise<void> {
    ...
    try {
      const credential = await this.database.client<{ accessTokenCiphertext: string | null }[]>`
      ...
      const errorCode = error instanceof ProfileSyncError ? error.code : 'PROFILE_SYNC_FAILED'
      await this.finishAuditRun(auditId, 'failed', errorCode)
      throw new Error(errorCode)
    }
  }

  private async findActiveMetaAccount(accountId: string): Promise<MetaAccount | null> {
  ...
  private async createAuditRun(account: MetaAccount, jobId: string | undefined): Promise<string> {
  ...
  private async persistProfile(account: MetaAccount, profile: ProfileSnapshot) {
    ...
    const nextMetadata = {
      ...account.metadata,
      profileSyncDueAt: new Date(now.getTime() + META_PROFILE_SYNC_DUE_INTERVAL_MS).toISOString(),
    }
  ...
  private async fetchProfile(account: MetaAccount, accessToken: string): Promise<ProfileSnapshot> {
    const fields =
    ...
    const url = new URL(`https://graph.facebook.com/v22.0/${encodeURIComponent(account.externalId)}`)
    url.search = new URLSearchParams({ fields, access_token: accessToken }).toString()
    ...
    if (!response.ok) {
      if (response.status === 401 || response.status === 403) throw new ProfileSyncError('GRAPH_UNAUTHORIZED')
      if (response.status === 429) throw new ProfileSyncError('GRAPH_RATE_LIMITED')
...
function parseFacebookProfile(payload: unknown): { id: string; name: string; pictureUrl: string | null } | null {
  if (!isRecord(payload) || typeof payload.id !== 'string' || typeof payload.name !== 'string') return null
  const picture = isRecord(payload.picture) && isRecord(payload.picture.data) ? payload.picture.data : null
  return {
    id: payload.id,
    name: payload.name,
...
function parseInstagramProfile(
  ...
  if (!isRecord(payload) || typeof payload.id !== 'string' || typeof payload.username !== 'string') return null
  return {
    id: payload.id,
    username: payload.username,
    profilePictureUrl: typeof payload.profile_picture_url === 'string' ? payload.profile_picture_url : null,
  }
}
